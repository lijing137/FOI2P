...
class VanillaConditionalTransformer(nn.Module):
    def __init__(self, blocks, d_model, num_heads, dropout=None, activation_fn='relu', return_attention_scores=False):
        ...
        for block in self.blocks:
            ...
            layers.append(TransformerLayer(d_model, num_heads, dropout, activation_fn))
...
class PEConditionalTransformer(nn.Module):
    def __init__(self, blocks, d_model, num_heads, dropout=None, activation_fn='relu', return_attention_scores=False):
        ...
        for block in self.blocks:
            _check_block_type(block)
            if block == 'self':
                layers.append(PETransformerLayer(d_model, num_heads, dropout, activation_fn))
            else:
                layers.append(TransformerLayer(d_model, num_heads, dropout, activation_fn))
        self.layers = nn.ModuleList(layers)
        self.return_attention_scores = return_attention_scores
...
class RPEConditionalTransformer(nn.Module):
    def __init__(
        self, blocks, d_model, num_heads, dropout=None,
        activation_fn='relu', return_attention_scores=False, parallel=False,
    ):
        ...
        for block in self.blocks:
            _check_block_type(block)
            if block == 'self':
                layers.append(RPETransformerLayer(d_model, num_heads, dropout, activation_fn))
            else:
                layers.append(TransformerLayer(d_model, num_heads, dropout, activation_fn))
        self.layers = nn.ModuleList(layers)
        self.return_attention_scores = return_attention_scores
