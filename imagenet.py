...
class ImgUpSample(nn.Module):
    def __init__(self, channels):
        super(ImgUpSample, self).__init__()
        self.up_conv = nn.Conv2d(channels, channels, kernel_size=1)

        self.unary = nn.Sequential(
            UnaryBlock(channels, channels),
            UnaryBlock(channels, channels)
        )
        self.output = UnaryBlock(channels, channels)

    def forward(self, feats_based, feats_inter):
        B, C, H, W = feats_based.size()

        F_up = F.interpolate(feats_inter, size=feats_based.shape[2:], mode='bilinear', align_corners=False)
        feats_based = feats_based.reshape(B, C, -1).permute(0, 2, 1)  # BxNxc
        F_up = F_up.reshape(B, C, -1).permute(0, 2, 1)  # BxNxc
        F_fused = self.output(self.unary(F_up) + feats_based)
        F_fused = F_fused.permute(0, 2, 1).reshape(B, C, H, W)
        return F_fused
