...
class CoFiI2P(nn.Module):
    def __init__(self,opt:Options):
        ...
        self.transformer1 = SpotGuidedAggregation(input_dim_l=128,
                                                  input_dim_h=128,
                                                  hidden_dim=128,
                                                  num_heads=4,
                                                  dropout=None,
        ...
        self.gamma1 = nn.Parameter(torch.tensor(0.1))  # 甚至初始化 0.01
        self.gamma2 = nn.Parameter(torch.tensor(0.1))  # 甚至初始化 0.01
    ...
    def forward(self,pc_data_dict,img, fine_center_kpt_coors,fine_xy, fine_pc_inline_index, mode):
        ...
        # pc encode and decode 这部分维度没有匹配
        pc_encode_feature_map = pc_feature_set[-1]  # [1280, 2048]
        ...
        img_pos = self.img_pos_encoding(img_xy)  # [1, 1280, 128]
        pc_pos = self.pc_pos_encoding(pc_data_dict['points'][-1].unsqueeze(0))  # [1280, 128]
        a = pc_data_dict['points'][-1]
        ...
        pc_feature_8 = pc_feature_middle + pc_pos # [b, n, c]
        # image_feature_mid, pc_features_fusion = self.transformer(image_feature_8, pc_feature_8)
        image_feature_mid, pc_features_fusion, spot_matching, cross_attn_im2pc, cross_attn_pc2im, score_im, idx_im, score_pc, idx_pc = self.transformer1(img,
                                                                pc_data_dict,
                                                                image_feature_8,
                                                                pc_feature_8,
                                                                image_feature_8,
        ...
        if mode == 'train' or mode == 'val':
            ...
            fine_pc_feature_patch = extract_pc_patch(pc_decode_3, fine_pc_inline_index, pc_data_dict['points'][-4])
        ...
        # 此处输出的特征用于后面计算loss——descri
        return (img_feature_norm, pc_feature_norm, coarse_img_score, coarse_pc_score, fine_img_feature_patch,
                fine_pc_inline_feature, fine_center_xy, coarse_pc_points, spot_matching, cross_attn_im2pc,
                cross_attn_pc2im, score_im, idx_im, score_pc, idx_pc, im_feature_loss, pc_feature_loss,
                fine_pc_feature_patch)
...
def extract_pc_patch(feature_map, center_points, neighbor_points, size=4):
    K = size * size
    center_coords = neighbor_points.index_select(0, center_points)
    dists = torch.cdist(center_coords.unsqueeze(0), neighbor_points.unsqueeze(0),p=2).squeeze(0)
    _, knn_idx = torch.topk(dists, k=K, largest=False, sorted=True)
    patch_feats = feature_map[knn_idx]
    return patch_feats
