...
sys.path.append(str(base_dir / 'model_data' / 'I2P'))
...
def test_acc(model, testdataloader, args):
    ...
    for step, data in enumerate(testdataloader):
        ...
        img_x = torch.linspace(0, coarse_img_mask.size(-1) - 1, coarse_img_mask.size(-1)).view(1, -1).expand(
            coarse_img_mask.size(-2), coarse_img_mask.size(-1)).unsqueeze(0).cuda()
        img_y = torch.linspace(0, coarse_img_mask.size(-2) - 1, coarse_img_mask.size(-2)).view(-1, 1).expand(
            coarse_img_mask.size(-2), coarse_img_mask.size(-1)).unsqueeze(0).cuda()
        # [2, 20, 64]
        ...
        img_features, pc_features, coarse_img_score, coarse_pc_score \
            ...
            , fine_center_xy, coarse_pc_points, spot_matching, \
            cross_attn_im2pc, cross_attn_pc2im, score_im, idx_im, \
            score_pc, idx_pc, im_feature_loss, pc_feature_loss, \
            fine_pc_feature_patch = model(pc_data_dict, img, fine_center_kpt_coors, fine_xy,
                                          fine_pc_inline_index, mode)  # [128, 20, 64] ,[128, 2560]

        pc_features_inline = torch.gather(pc_features, index=pc_kpt_idx.expand(pc_features.size(0), args.num_kpt),
                                          dim=-1)
        ...
        pc_xyz_inline = torch.gather(pc_data_dict['points'][-1].T,
                                     index=pc_kpt_idx.unsqueeze(0).expand(3, args.num_kpt), dim=-1)

        img_features_flatten = img_features.contiguous().view(img_features.size(1), -1)

        img_xy_flatten = img_xy.contiguous().view(2, -1)
        img_features_flatten_inline = torch.gather(img_features_flatten, index=coarse_img_kpt_idx.unsqueeze(0).expand(
            img_features_flatten.size(0), args.num_kpt), dim=-1)
        img_xy_flatten_inline = torch.gather(img_xy_flatten,
                                             index=coarse_img_kpt_idx.unsqueeze(0).expand(2, args.num_kpt), dim=-1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_path', type=str, default='/media/hnu/MyPassport/Li_Jing/dataset/kitti/',
                        metavar='data_path',
    ...
    parser.add_argument('--save_path', type=str, default=str(base_dir / 'model_data' / 'I2P' / 'model'), metavar='save_path',
    ...
    writer = SummaryWriter(
        log_dir=os.path.join(str(base_dir / 'model_data' / 'I2P' / 'runs'), datetime.now().strftime("%Y%m%d-%H%M%S")))
    ...
    trainloader = torch.utils.data.DataLoader(train_dataset, batch_size=args.train_batch_size, shuffle=True,
    ...
    # optimizer = torch.optim.Adam(learnable_params, lr=current_lr, weight_decay=1e-6)
    ...
    for epoch in range(args.epoch):
        for step, data in enumerate(trainloader):
            ...
            mode = 'train'
            optimizer.zero_grad()
            # 此处从SwinTransformer和KpnConv中获取图像和点云特征
            img = data['img'].cuda() #获取的图像特征有6个尺寸
            # pc = data['pc'].cuda()  #full size
            # intensity = data['intensity'].cuda()
            # sn = data['sn'].cuda()
            pc_data_dict = data['pc_data_dict']  #获取的点云特征有4个尺寸
            for key in pc_data_dict:   #读取字典中的数据
                for j in range(len(pc_data_dict[key])):
                    pc_data_dict[key][j] = torch.squeeze(pc_data_dict[key][j]).cuda() # 去除无效维度，将1*1*5*128变为5*128
            pc_data_dict['feats'] = torch.squeeze(pc_data_dict['feats']).cuda()
            K_4 = torch.squeeze(data['K_4'].cuda())  # K可能代表图像变为原来1/4尺寸的相机内参矩阵
            K = torch.squeeze(data['K'].cuda())  # K可能代表相机内参矩阵
            P = torch.squeeze(data['P'].cuda())  # P可能代表投影矩阵，将3D点投影到2D图像平面
            # pc_mask=data['pc_mask'].cuda()  
            # img_mask=torch.squeeze(data['img_mask']).cuda()     #1/4 size
            # 得到一个粗略图像掩码，其尺寸是原始图像的1/4，掩码的作用·······
            coarse_img_mask = torch.squeeze(data['coarse_img_mask']).cuda()  # [20, 64]
            # B=coarse_img_mask.size(0)
            pc_kpt_idx = torch.squeeze(data['pc_kpt_idx']).cuda()  # (128) # pc_kpt_idx可能代表点云关键点的索引
            pc_outline_idx = torch.squeeze(data['pc_outline_idx']).cuda()  # (128) # pc_outline_idx可能代表点云外轮廓的索引
            # img_kpt_idx=torch.squeeze(data['img_kpt_idx']).cuda()
            fine_img_kpt_index = torch.squeeze(data['fine_img_kpt_index']).cuda()  # [128] # fine_img_kpt_index可能代表精细图像关键点的索引

            coarse_img_kpt_idx = torch.squeeze(data['coarse_img_kpt_idx']).cuda()  # [128] # coarse_img_kpt_idx可能代表粗略图像关键点的索引
            # img_outline_idx=torch.squeeze(data['coarse_img_outline_index']).cuda()  # [128]
            fine_center_kpt_coors = torch.squeeze(data['fine_center_kpt_coors']).cuda()  # [2 64] # 可能代表精细中心关键点坐标（xyz）
            fine_xy = torch.squeeze(data['fine_xy_coors']).cuda() # 可能代表精细图像坐标
            fine_pc_inline_index = torch.squeeze(data['fine_pc_inline_index']).cuda() # 精细点云内部索引
            ...
            img_xy = torch.cat((img_x, img_y), dim=0) # 创建每个像素的坐标信息，以生成图像坐标网格
            # model_start = time.time()
            # 将诸多数据输入model获得8个输出变量：图像特征、点云特征、粗略图像评分、粗略点云评分、精细图像特征、精细点云内部特征、精细中心关键点xy坐标、粗略点云位置。
            img_features, pc_features, coarse_img_score, coarse_pc_score \
                , fine_img_feature_patch, fine_pc_inline_feature \
                , fine_center_xy, coarse_pc_points,spot_matching, \
                cross_attn_im2pc, cross_attn_pc2im, score_im, idx_im, \
                score_pc, idx_pc, im_feature_loss, pc_feature_loss, \
                fine_pc_feature_patch = model(pc_data_dict, img, fine_center_kpt_coors, fine_xy,
                                                           fine_pc_inline_index, mode)  # [1, 128, 20, 64] ,[128, 2560]
            # model_end = time.time()
            # print('model calculate time:', model_end - model_start)
            '''
            coarse match and overlap detection
            '''
            # [128, 128]  描述子损失函数包括这个，请分析
            pc_features_inline = torch.gather(pc_features, index=pc_kpt_idx.expand(pc_features.size(0), args.num_kpt),
                                              dim=-1)
            # [128, 128]
            pc_features_outline = torch.gather(pc_features,
                                               index=pc_outline_idx.expand(pc_features.size(0), args.num_kpt), dim=-1)
            # [3, 128]
            pc_xyz_inline = torch.gather(pc_data_dict['points'][-1].T,
                                         index=pc_kpt_idx.unsqueeze(0).expand(3, args.num_kpt), dim=-1)
            # [128, 1280]
            ...
            img_xy_flatten = img_xy.contiguous().view(2, -1)
            # [128, 128] 描述子损失函数包括这个，请分析
            img_features_flatten_inline = torch.gather(img_features_flatten,
                                                       index=coarse_img_kpt_idx.unsqueeze(0).expand(
                                                           img_features_flatten.size(0), args.num_kpt), dim=-1)
            # [2, 128]
            img_xy_flatten_inline = torch.gather(img_xy_flatten,
                                                 index=coarse_img_kpt_idx.unsqueeze(0).expand(2, args.num_kpt), dim=-1)
            ...
            # loss_desc是论文中的L_coarse
            loss_desc, dists = desc_loss(img_features_flatten_inline, pc_features_inline, correspondence_mask,
                                         pos_margin=args.pos_margin, neg_margin=args.neg_margin)

            loss_spot = spot_guided_loss(coarse_img_kpt_idx, pc_kpt_idx, correspondence_mask, spot_matching)

            # loss_desc.retain_grad()
            ...
            writer.add_scalars('pc_score',
                               {'inline_max': coarse_pc_inline_score.max(), 'inline_min': coarse_pc_inline_score.min(),
                                'inline_avg': torch.mean(coarse_pc_inline_score),
                                'outline_max': coarse_pc_outline_score.max(),
                                'outline_min': coarse_pc_outline_score.min(),
                                'outline_avg': torch.mean(coarse_pc_outline_score)}, global_step)
            # loss_2是论文中的L_classify
            ...
            loss_dual = fine_circle_loss_im2pc(fine_img_feature_patch, relative_index, fine_pc_feature_patch, 64)
            wei = 0.5
            loss_3 = wei * loss_3 + (1 - wei) * loss_dual


            # loss = loss_desc + loss_2 + loss_3
            loss = 0.2 * loss_spot + loss_desc + loss_2 + loss_3
            ...
            end_time = time.time()
            # print('total time:',1 end_time-start_time)
            # torch.cuda.empty_cache()
            # print(loss)
            writer.add_scalar('Spot Loss:', loss_spot, global_step)
            ...
            # coarse_logits = loss_desc
            # if step % 10 == 0:  ### �� 频率随意
            #     if coarse_logits.grad is None:
            #         grad_info = 'No grad'
            #     else:
            #         grad_info = f'{coarse_logits.grad.abs().mean():.4e}'
            #     print(f"[E{epoch:02d} S{step:04d}] "
            #           f"coarse μ={coarse_logits.mean():.3f} σ={coarse_logits.std():.3f} | "
            #           f"grad_abs_mean={grad_info}")

            if global_step % 10 == 0:
                # logger.info('%s-%d-%d, loss: %f, loss_desc: %f, loss_2: %f, loss_3: %f' % (
                # 'train', epoch, global_step, loss.data.cpu().numpy(), loss_desc.data.cpu().numpy(),
                # loss_2.data.cpu().numpy(), loss_3.data.cpu().numpy()))

                logger.info('%s-%d-%d, loss: %f, ls_sp: %f, '
                            'ls_de: %f, loss_2: %f, loss_3: %f' % (
                    'train', epoch, global_step, loss.data.cpu().numpy(),loss_spot.data.cpu().numpy(),
                    loss_desc.data.cpu().numpy(), loss_2.data.cpu().numpy(), loss_3.data.cpu().numpy()))

            if global_step % args.val_freq == 0:
                acc = test_acc(model, testloader, args)
                logger.info('acc: top5 %s %s %s %s %s', acc[0].cpu().numpy(), acc[1].cpu().numpy(),
                            acc[2].cpu().numpy(), acc[3].cpu().numpy(), acc[4].cpu().numpy())
                writer.add_scalars('test_acc/topk',
                                   {'k=1': acc[0], 'k=2': acc[1], 'k=3': acc[2], 'k=4': acc[3], 'k=5': acc[4]},
                                   int(global_step / 100))
