...
if __name__ == '__main__':
    ...
    ckpt_dir = str(base_dir / 'model_data' / 'I2P' / 'model')
    eval_dir = str(base_dir / 'model_data' / 'I2P' / 'eval_results')
    epoch_list = list(range(23, 25))
    # model.load_state_dict(torch.load(str(base_dir / 'model_data' / 'I2P' / 'model' / 'mode_epoch_9.t7')))

    for ep in epoch_list:
        ckpt_path = os.path.join(ckpt_dir, f'mode_epoch_{ep}.t7')
        ...
        out_dir = str(base_dir / 'model_data' / 'I2P' / 'FeatsMap')
        ...
        with torch.no_grad():
            for step, data in enumerate(testloader):
                ...
                img_features, pc_features, coarse_img_score, coarse_pc_score \
                    ...
                    , fine_center_xy, coarse_pc_points, spot_matching, cross_attn_im2pc, cross_attn_pc2im, score_im, idx_im, score_pc, idx_pc = model(
                    pc_data_dict, img, fine_center_kpt_coors, fine_xy,
                    fine_pc_inline_index, mode)
                # steps_to_vis = {1}  # 用 set 查找 O(1)
                # if step in steps_to_vis:
                #     # out = visualize_img_pc_features(img_features, pc_features, step,
                #     #                                 out_dir=out_dir, upsample_to=None)
                #     paths = visualize_per_channel(
                #         img_features,  # [1,128,20,64]
                #         pc_features,  # [128,1280]
                #         step=1200,
                #         out_dir=out_dir,
                #         img_hw=(20, 64),
                #         pc_hw=(20, 64),  # 若你的 1280 不对应 20×64，可改成 None
                #         cmap="gray"  # 或 "magma"
                #     )
                ...
                dist = torch.cosine_similarity(fine_img_feature_patch.unsqueeze(-1),
                                               fine_pc_inline_feature.unsqueeze(-2))
                ...
                is_success, R, t, inliers = cv2.solvePnPRansac(cameraMatrix=K.cpu().numpy(),
                                                               imagePoints=fine_xy.T.cpu().numpy(),
                                                               objectPoints=coarse_pc_points.cpu().numpy(),
                                                               iterationsCount=10000, distCoeffs=None)
                ...
                np.save(os.path.join(str(base_dir / 'model_data' / 'I2P' / 'eval_results'), '%06d' % (step)), save_dict)
            ...
            np.save(f'epoch_{ep}_qnorm_t_error.npy', t_diff_set)
            np.save(f'epoch_{ep}_qnorm_r_error.npy', angles_diff_set)
